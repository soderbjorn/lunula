 ...
 * (v2 custom theme definitions shared with every Lunula app) and the
 * per-app `<DarknessDir>/lunula-demo.json` (selections + UI prefs).
 * Layout state stays per-app under `<DarknessDir>/LunulaDemo/`. The
 * toolkit owns the JSON shapes; this host owns where the bytes
 * physically land. */
package se.soderbjorn.lunula.demo.electron
...
private const val APP_NAME = "LunulaDemo"
 ...
 */
private const val APP_NAME_KEBAB = "lunula-demo"
 ...
 * cross-app `themes.json` / per-app `lunula-demo.json` because
...
/**
 * Cross-app shared lunula themes file path: holds the v2 custom
 * themes (`PersistKeys.THEME_V2_CUSTOM`) — read/written by every
 * Lunula app on this machine.
 */
...
/** Per-app lunula layout-state file path. Stays under the app's own subdir. */
...
/**
 * Resolve a path relative to the OS-conventional Lunula data
 * directory (the same root every Lunula app on this machine uses).
 *
 * - macOS: `~/Library/Application Support/Lunula/<filename>`
 * - Windows: `%APPDATA%\Lunula\<filename>`
 * - Linux: `$XDG_CONFIG_HOME/lunula/<filename>` (defaults to
 *   `~/.config/lunula/`).
 */
...
private fun createWindow() {
    ...
    // lunula's `autoApplyCustomTitleBarBodyClass` (called from
    // `injectLunulaStyles`) can deterministically toggle
    ...
    // automatically by lunula's `autoWireMacFullscreenBodyClass`
    // (called from `injectLunulaStyles`), so the only host
...
private fun registerIpcHandlers() {
    ...
    // lunula-core's [SHARED_THEMES_KEYS]; we mirror it here at the disk
    // boundary.
    ...
    // reconstructed from disk (`themes.json`, `lunula-demo.json`,
