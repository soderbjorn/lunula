...
/**
 * Alpha-composites [top] over the opaque [base] and returns an opaque
 * ARGB colour representing what the eye would actually see when [top] is
 * painted on a surface coloured [base].
 *
 * Used by the theme resolver to flatten any palette token whose downstream
 * consumers cannot agree on whether to honour or strip alpha — most
 * notably [terminal.bg], which is consumed both as a CSS background
 * (translucent variant blends through to the surface beneath
 * `.terminal-inner`) and as xterm.js's `theme.background` (alpha is
 * silently dropped, producing the unblended pure colour). Pre-flattening
 * at the resolver gives both consumers the same opaque value and
 * eliminates the visible "double-border" seam that would otherwise appear
 * inside the terminal pane.
 *
 * If [top] is already fully opaque the function returns it unchanged.
 * The alpha channel of [base] is ignored — callers must guarantee that
 * the composite base is itself opaque (in practice the resolver passes
 * its `bg` seed, which is always `0xFF`-alpha by construction).
 *
 * @param top the colour painted on top, possibly with alpha < `0xFF`
 * @param base the opaque colour painted beneath [top]
 * @return the composited colour, always with alpha = `0xFF`
 * @see argbToHex
 * @see argbToCss
 */
fun flattenOnto(top: Long, base: Long): Long {
    val a = ((top shr 24) and 0xFF).toInt()
    if (a == 0xFF) return top
    val t = a / 255.0
    fun ch(shift: Int): Long {
        val ct = ((top shr shift) and 0xFF).toDouble()
        val cb = ((base shr shift) and 0xFF).toDouble()
        return (cb + (ct - cb) * t).toInt().coerceIn(0, 255).toLong()
    }
    return 0xFF000000L or (ch(16) shl 16) or (ch(8) shl 8) or ch(0)
}
