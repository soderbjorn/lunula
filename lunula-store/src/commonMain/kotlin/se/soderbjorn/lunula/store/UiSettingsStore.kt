/**
 * Filesystem helpers for persisting Lunula UI/theme state to a shared
 * location.
 *
 ...
 * an "Application Support" / "AppData" / "config" directory that any Lunula
 * app on the same machine can read, and [readUiSettingsRaw] /
 * [writeUiSettingsRaw] round-trip the raw JSON document through that file.
 *
...
/**
 * Conventional filename for the shared Lunula theme/scheme definitions
 * file (custom themes, custom schemes, favorites). Cross-app: every
 * Lunula app on the same machine reads/writes this single file.
 *
 ...
 * Lunula apps store their shared theme/UI state. The literal name is
 * historical ("Darkness") and is deliberately NOT renamed: it is an
 * on-disk location every installed app already reads and writes.
 */
const val UI_SETTINGS_DIR_NAME = "Darkness"

/**
 * Returns the conventional path where Lunula's shared theme/scheme
 * definitions live on the current platform.
 *
 * - **macOS**: `~/Library/Application Support/Lunula/themes.json`
 * - **Windows**: `%APPDATA%\Lunula\themes.json`
 * - **Linux**: `$XDG_CONFIG_HOME/lunula/themes.json` (defaults to `~/.config/lunula/`)
 * - **Android**: `<context.filesDir>/Lunula/themes.json` — see Android-specific helper
 * - **iOS**: `<Application Support>/Lunula/themes.json` via NSFileManager
 *
 ...
 * fonts, sizes, app-specific toggles) and is *not* shared across Lunula
 * apps. Each app passes its own short, lower-kebab-case identifier
 * (e.g. `"termtastic"`, `"notegrow"`, `"lunula-demo"`).
 *
 * - **macOS**: `~/Library/Application Support/Lunula/<appName>.json`
 * - **Windows**: `%APPDATA%\Lunula\<appName>.json`
 * - **Linux**: `$XDG_CONFIG_HOME/lunula/<appName>.json`
...
/**
 * Reads the raw file contents without parsing. Apps own the JSON schema
 * (theme selection/custom themes plus any app-private keys); this helper
 * just hands back the bytes as text so nothing is dropped on a round-trip.
 *
...
/**
 * Atomic-write a raw JSON payload verbatim, creating parent directories as
 * needed.
 *
 * Implementations must be **atomic from a reader's point of view**: a
 * concurrent or external [readUiSettingsRaw] either sees the previous file
 ...
 *
 * Records the written bytes so a co-resident [watchUiSettings] won't bounce
 * on the writer's own event.
 *
...
/**
 * Watches [path] for external changes and invokes [onChange] with the raw
 * file contents whenever the file is modified by another process (or another
 ...
 * call to [writeUiSettingsRaw] from the same process, so a writer that also
 ...
 *                 (iOS) with the raw file contents — callers must marshal
 *                 back to whatever thread they care about themselves
