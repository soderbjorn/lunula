/**
 * Semantic colour token structure for the Darkness theme system.
 *
...
data class ResolvedPalette(
     ...
     *
     * Per-pane active-state titlebar colours (when [Sections.ActiveChrome] is
     * assigned) are not represented on this struct — they're projected
     * directly as CSS vars `--t-chrome-titlebar-active` / `--t-chrome-titleText-active`
     * by `applyUiSettings`, sourced from the ActiveChrome section's resolved
     * palette. CSS rules consume them with a fallback to the inactive vars.
     *
