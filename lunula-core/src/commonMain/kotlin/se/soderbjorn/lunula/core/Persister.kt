 ...
 *
 * ### Callers
 * Held by [se.soderbjorn.lunula.web.shell.AppShellSpec] and read /
...
object PersistKeys {
     ...
     * mode: per-tab active [se.soderbjorn.lunula.web.layout.LayoutPreset]
    ...
    /**
     * Serialized [se.soderbjorn.lunula.store.WorldsState] — the "worlds"
     * container one level above [se.soderbjorn.lunula.store.LayoutState].
     ...
     * across Lunula apps via the `themes.json` file (see [SHARED_THEMES_KEYS]).
     */
     ...
     * `HotkeyBindings.applyCustomBindingsJson` in lunula-web).
     */
