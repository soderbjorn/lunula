 ...
 * classes shipped in `lunula.css`; consumers must call
 * `injectLunulaStyles()` once at boot.
 *
 * @see PaneHeaderSpec
...
import se.soderbjorn.lunula.web.util.encircledIndexGlyph
 ...
 * [se.soderbjorn.lunula.web.shell.DT_TAB_DRAG_MIME] so foreign drags
 * (text, files) and tab reorders cannot be mistaken for pane reorders.
 */
const val DT_PANE_DRAG_MIME: String = "application/x-lunula-pane"
...
data class PaneHeaderSpec(
     ...
     * blank). Wired from [se.soderbjorn.lunula.web.shell.AppShellSpec.allowEmptyPaneRename].
     */
