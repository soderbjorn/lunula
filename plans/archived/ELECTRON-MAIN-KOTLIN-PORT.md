...
- **The renderer side already supports it.** `lunula-web` (browser-only) exposes `Persister` as an interface. `ElectronIpcPersister` is one impl; a website consumer plugs in `LocalStoragePersister` (or any other). Nothing in `lunula-core` or `lunula-web` requires `darknessApi` to exist — `ElectronIpcPersister` is only constructed when the consumer chose Electron. The browser bundle has no Node imports today, and this plan does not add any.
- **The new Node-target code is opt-in and isolated.** The `jsNodeMain` source set lives in the demo app's own module (or, in the follow-up, a separate `toolkit-electron-main` module with its own `js { nodejs() }` target). Web-only consumers depend only on `lunula-core` + `lunula-web` and never see the Node code, its `external` Electron declarations, or its npm dependencies.
...
2. **Port `main.js` to `ElectronMain.kt`** in `jsNodeMain`:
   ...
   - Reuse the path-resolution code shape from `UiSettingsStore.jvm.kt` (mac/windows/linux branches, `Library/Application Support/Lunula/<APP_NAME>/`).
...
- **Modify** `/Users/soderbjorn/repo/lunula/darkness-demo/develop/web/build.gradle.kts` — add Node target, source set wiring.
- **Add** `/Users/soderbjorn/repo/lunula/darkness-demo/develop/web/src/jsCommonMain/kotlin/se/soderbjorn/lunula/demo/IpcChannels.kt` — channel-name constants, argv-prefix constants, `APP_NAME`.
- **Add** `/Users/soderbjorn/repo/lunula/darkness-demo/develop/web/src/jsNodeMain/kotlin/se/soderbjorn/lunula/demo/electron/ElectronMain.kt` — the port of `main.js`.
- **Add** `/Users/soderbjorn/repo/lunula/darkness-demo/develop/web/src/jsNodeMain/kotlin/se/soderbjorn/lunula/demo/electron/ElectronExternals.kt` — `external` declarations for the Electron / Node APIs actually used.
- **Modify** `/Users/soderbjorn/repo/lunula/darkness-demo/develop/electron/main.js` — shrink to a single `require(...)` line.
- **Modify** `/Users/soderbjorn/repo/lunula/darkness-demo/develop/electron/build.gradle.kts` — add `copyMainBundle` task; wire as a dependency of `run` and `dist`.
- **Optionally modify** `/Users/soderbjorn/repo/lunula/darkness-demo/develop/electron/preload.js` — port to Kotlin/JS as a second Node webpack entry, or leave as JS (small, rarely changes).

## Reuse from existing code

- **Path-resolution logic shape**: mirror `UiSettingsStore.jvm.kt` in `lunula/develop/lunula-store/src/jvmMain/kotlin/se/soderbjorn/lunula/store/UiSettingsStore.jvm.kt` — same OS branches, same directory layout (`Library/Application Support/Lunula/<APP_NAME>/`), same filenames.
- **IPC contract**: the renderer side already consumes `globalThis.darknessApi.{readUiSettings,writeUiSettings,readLayoutState,writeLayoutState}` via `ElectronIpcPersister.kt` in `lunula/develop/lunula-web/src/jsMain/kotlin/se/soderbjorn/lunula/web/ElectronIpcPersister.kt`. The Kotlin port of `main.js` must register exactly these channels — don't change names or shapes.
...
4. `ls ~/Library/Application\ Support/Lunula/DarknessDemo/` shows `ui-settings.json` and `layout-state.json` — same path as before the port.
...
If this lands cleanly, lift `ElectronMain` into a new `lunula` module (`toolkit-electron-main`) so notegrow and termtastic can reuse it. notegrow needs additional `notegrow:*` IPC handlers for file ops; termtastic embeds a JVM server jar — both can extend the toolkit base.
