/**
 * Deterministic palette derivation for the Darkness semantic theme system.
 *
...
// results on saturated low-luminance fg (Vapor pink, Plasma, Neon Green).
...
fun ColorScheme.resolve(isDark: Boolean): ResolvedPalette {
    ...
    // ── Terminal ───────────────────────────────────────────────────
    // terminal.bg must be opaque: the CSS path (argbToCss) and the
    // xterm.js path (argbToHex) disagree on translucent values — CSS
    // composites through to whatever sits behind .terminal-inner while
    // xterm silently strips alpha — producing a visible inner-ring seam.
    // Flatten any alpha onto the opaque seed bg so both consumers see
    // the same colour. See darkness-toolkit/terminal-bg-mismatch-fix.md.
    ...
    val chromeTitleText = overrideFor(ovr, "chrome.titleText", isDark) ?: fg
    // Pane titles are rendered at 11px, uppercase, with letter-spacing
    // (`.dt-pane-title` in `darkness-toolkit.css:1359-1381`) — far less
    // ink per glyph than the sidebar's lowercase 13px rows. To read at
    // the same perceived weight on a same-scheme chrome surface (e.g.
    // Emerald Garden uses Forest Panel for both `sidebar` and `chrome`),
    // the title needs more contrast, not equal contrast. Pure `fg`
    // gives the strongest reading without overshooting into an alien
    // hue, and falls through naturally on every scheme — light fg over
    // a dark titlebar reads bright; dark fg over a light titlebar reads
    // crisp. Schemes that want softer titlebar text can still override
    // `chrome.titleText` explicitly.
