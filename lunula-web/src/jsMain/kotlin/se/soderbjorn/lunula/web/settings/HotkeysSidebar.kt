/**
 * HotkeysSidebar — slide-in right-side panel whose **body is supplied by
 * the host app**, dedicated to showing a keyboard-shortcut reference.
 *
 * A fourth toolkit right-sidebar, sibling to [SettingsSidebar] (appearance
 * / fonts), `ThemeManagerSidebar` (theme editor) and [AppSettingsSidebar]
 * (app-specific preferences). It exists so each Lunula app can surface a
 * nicely-formatted hotkeys cheat-sheet through the same chrome, animation
 * and mutual-exclusion contract as the other panels, while the toolkit
 * stays agnostic about the actual shortcut list (which is app-specific).
 *
 * Unlike the other three panels there is no dedicated topbar button: the
 * host opens this sidebar programmatically via
 * `AppShellHandle.openHotkeysSidebar()` (wired to an in-app "Hotkeys"
 * button and, on Electron, a menu item). Mutual exclusion with the other
 * three right-sidebars is enforced in `AppShellMount`: opening any panel
 * closes whichever sibling is currently mounted.
 *
 * Public API mirrors [AppSettingsSidebar] so the shell wires it the same way:
 *   - [toggleHotkeysSidebar] flips state and asks the host to rebuild.
 *   - [isHotkeysSidebarOpen] is consulted by the host's rebuild path.
 *   - [closeHotkeysSidebar] force-closes it (used for mutual exclusion).
 *   - [buildHotkeysSidebar] returns the freshly-mounted `<aside>` once the
 *     host's rebuild reaches the right-sidebar slot.
 *
 * @see HotkeysSidebarSpec
 * @see se.soderbjorn.lunula.web.shell.AppShellSpec.hotkeysContent
 */
...
/**
 * Spec passed to [buildHotkeysSidebar].
 *
 * @property title       header label shown at the top of the sidebar
 *   (apps typically pass "Keyboard shortcuts").
 ...
 */
data class HotkeysSidebarSpec(
    val title: String,
    val bodyFactory: () -> HTMLElement,
)

/** True while the Hotkeys sidebar is considered open. */
...
/**
 * Latest `requestRebuild` callback handed to [toggleHotkeysSidebar].
 *
...
/**
 * Whether the Hotkeys sidebar is currently open. The host's rebuild path
 * consults this to decide whether to mount the sidebar.
 *
 * @return `true` while the sidebar is open or in the process of closing.
 */
fun isHotkeysSidebarOpen(): Boolean = sidebarOpen

/**
 * Toggle the Hotkeys sidebar open/closed.
 *
 ...
 *   AppFrame; the sidebar slot consults [isHotkeysSidebarOpen] on each pass
 *   and either mounts or omits this panel accordingly.
 */
fun toggleHotkeysSidebar(requestRebuild: () -> Unit) {
...
/**
 * Force-close the Hotkeys sidebar synchronously (no animation).
 *
...
/**
 * Build the right-sidebar `<aside>` element that hosts the Hotkeys panel.
 *
 ...
 * @see HotkeysSidebarSpec
 */
fun buildHotkeysSidebar(spec: HotkeysSidebarSpec): HTMLElement {
    ...
    if (isReMount) {
        ...
        renderHotkeysBody(mountTarget, spec)
    } else {
    ...
    val handler: (Event) -> Unit = { ev ->
        ...
        if (key == "Escape") {
            ...
            if (rebuild != null) toggleHotkeysSidebar(rebuild)
        }
    }
 ...
 * the Hotkeys panel reads visually identical to its siblings.
 */
private fun renderHotkeysBody(target: HTMLElement, spec: HotkeysSidebarSpec) {
    target.innerHTML = ""
