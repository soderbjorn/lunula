 ...
 * darkness-toolkit.css) and the surrounding chrome grows vertically to
 * accommodate.
 *
 ...
 * in `darkness-toolkit.css`; consumers must call `injectDarknessToolkitStyles()`
 * once at boot.
 *
 * @see TabBarSpec
 * @see renderTabBar
 ...
 * tab reordering.
 */
const val DT_TAB_DRAG_MIME: String = "application/x-darkness-tab"
...
class TabBarCallbacks(
     ...
     * `application/x-darkness-pane` MIME shipped by the toolkit's pane
...
private fun wireTabAsPaneDropTarget(el: HTMLElement, tabId: String, spec: TabBarSpec) {
    ...
    el.addEventListener("drop", { ev ->
        ...
        el.classList.remove(TabBarClassNames.TAB_DROP_BEFORE)
        val dt = ev.asDynamic().dataTransfer ?: return@addEventListener
        val sourceId = (dt.getData("application/x-darkness-pane") as? String)
...
private fun isPaneDragOver(dynamicEv: dynamic): Boolean {
    ...
    for (i in 0 until len) if (types[i] == "application/x-darkness-pane") return true
    return false
}
